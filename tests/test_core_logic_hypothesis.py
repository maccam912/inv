...
def test_predict_leftover_quantity_edge_cases():
    ...
    # Case 1: No inventory
    result = predict_leftover_quantity(
        session, lot_number=lot_number, site_name=site_name
    )
    assert result is None
    ...
    result = predict_leftover_quantity(
        session, lot_number=expired_lot_number, site_name=site_name
    )
    # All current inventory is leftover since it's already expired
    assert result == EXPIRED_INVENTORY_AMOUNT
